...
Any Strands-supported model provider works. `warmintro/config.py` picks the backend at runtime from `WARMINTRO_MODEL_PROVIDER`: **Anthropic API** directly (default, fastest local setup, no AWS account needed) or **Amazon Bedrock** (`WARMINTRO_MODEL_PROVIDER=bedrock`, using standard AWS credential resolution — env vars, an AWS CLI profile, or an IAM role). No agent code changes when you switch — `agents.py` only ever calls `get_model()` (see [Strands' model provider docs](https://strandsagents.com/docs/user-guide/concepts/model-providers/)).
